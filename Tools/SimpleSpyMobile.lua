...
Simple.Text = "SimpleSpy For Mobile[小云翻译的]"
...
newButton("函数信息", function()
        ...
        if selected then
                ...
                TextLabel.Text = "完成！函数信息已由 SimpleSpy 序列化器生成。"
        end
end)
...
--- 将选中的 Log 远程事件从 RemoteSpy 中排除
newButton("排除 (i)", function()
        return "点击排除此远程事件。\n排除远程事件后，SimpleSpy 将忽略它，但它仍然可用。"
end, function()
        if selected then
...
--- 阻止选中的远程事件发送到服务器（仍会记录日志）
newButton("阻止 (i)", function()
        return "点击阻止该远程事件发送。\n阻止远程事件后，它仍会被 SimpleSpy 记录，但不会继续发送到服务器。"
end, function()
        if selected then
                if selected.Remote.remote then
                        ...
                        TextLabel.Text = "错误！实例可能不再存在，尝试使用 阻止 (n)。"
                end
        end
end)

--- 阻止所有具有相同名称的远程事件发送到服务器（仍会记录日志）
newButton("阻止 (n)", function()
        return "点击阻止具有相同名称的远程事件发送。\n阻止远程事件后，它仍会被 SimpleSpy 记录，但不会继续发送到服务器。"
end, function()
        if selected then
...
newButton("反编译", function()
        ...
        if selected then
                if selected.Source then
                        ...
                        TextLabel.Text = "未找到源代码！"
                end
        end
end)

newButton("禁用函数信息", function()
        return string.format(
                "[%s] 切换函数信息显示（因为它在某些游戏中可能导致卡顿）",
                funcEnabled and "启用" or "禁用"
        )
end, function()
...
newButton("自动阻止", function()
        return string.format(
                "[%s] [测试版] 智能检测并排除日志中的频繁远程调用",
                autoblock and "启用" or "禁用"
        )
end, function()
...
newButton("调用脚本", function()
        return string.format(
                "[%s] [不安全] 使用 'getcallingscript' 来获取反编译和获取脚本的调用脚本。更可靠，但可能使 SimpleSpy 被检测到或导致不稳定。",
                useGetCallingScript and "启用" or "禁用"
        )
end, function()
...
newButton("键转字符串", function()
        return string.format(
                "[%s] [测试版] 使用一个实验性的新功能，模拟 Roblox 在表中使用非原始类型作为键的行为。仍在开发中，可能无法正确反映 tostring 处理的（空）userdata。",
                keyToString and "启用" or "禁用"
        )
end, function()
...
newButton("切换返回值记录", function()
        return string.format(
                "[%s] [实验性] 启用记录 'GetReturnValue' 的返回值\n\n使用此方法需自行承担风险，因为它可能会被检测到。",
                recordReturnValues and "启用" or "禁用"
        )
end, function()
...
newButton("获取返回值", function()
        return "[实验性] 如果启用了 '返回值记录'，此操作将显示 RemoteFunction 的记录返回值（如果可用）。"
end, function()
        if selected then
